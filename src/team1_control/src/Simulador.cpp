...
#define TEMPO_PARA_FREEBALL 2000000
...
ros::Publisher pubAtacanteHome;
ros::Publisher pubVolanteHome;
ros::Publisher pubGoleiroHome;
...
char wndname[] = "Controle Time 1 VSSS";
...
void EnviaCmd2Gazebo(int indiceRobo, float ve, float vd, float ang) {
	...
	// float vel_linear_robo =  (vd + ve) / 2 / 100;
	// float vel_angular_robo = (RAIO_DA_RODA_M / DIST_ENTRE_RODAS_M) * (vd - ve) * 30;
	// float vel_angular_robo = ((ve*21) / (2*3.45*PI)) * 2 ;
	// float vel_angular_robo = (vd - ve) / DIST_ENTRE_RODAS_M;
	
	// float vel_angular = (vd - ve) / DIST_ENTRE_RODAS_M;
	// float dang = vel_angular + estadoReal[indiceRobo].angulo * (PI / 180);
	// // float dang = angulo - (estadoReal[indiceRobo].angulo * (PI / 180));
	// float vel_angular_robo = dang;
	
	// float vel_angular_robo = (RAIO_DA_RODA_M / DIST_ENTRE_RODAS_M) * (vd - ve);
	// float vel_angular_robo = ang * PI / 180;
	// float dang_rad = atan2((vd - ve), DIST_ENTRE_RODAS_M); // bom
	// float vel_angular_robo = ((vd - ve) / DIST_ENTRE_RODAS_M) * 2.14; // TESTE SEM ATAN
	// float vel_angular_robo = atan2((vd - ve), DIST_ENTRE_RODAS_M) * 2.14; // FUNCIONOU
	// float vel_angular_robo = (vd - ve) * (VEL_MAXIMA_ROBO_CM/100 / 7) / DIST_ENTRE_RODAS_M;
	float vel_angular_robo = (vd - ve) * (VEL_MAXIMA_ROBO_CM/100 / 7) / DIST_ENTRE_RODAS_M;
// 	VEL_MAXIMA_ROBO_CM 150.0 // velocidade maxima de robo em cm
// #define VEL_MAXIMA_ROBO_UNID 7
			// float ang_rad = (estadoReal[indiceRobo].angulo * (PI / 180)) + dang_rad;
			// float vel_angular_robo = dang_rad;
			// ang = (estadoReal[i].angulo += dang);

			//objetivo 5.75 segundos

		msg.linear.x = vel_linear_robo;
...
void enviaDados(unsigned char b1, unsigned char b2, unsigned char b3, unsigned char b4, unsigned char b5, unsigned char b6) {
	...
	for (i = 0; i < 3; i++) {
		...
		vd = somaVd / cont;
		printf("\nvd = somaVd / cont: %0.2f ### somavd: %f ### count: %f", vd, somaVd, cont); // 0.00
		ve = somaVe / cont;
		printf("\nve = somaVe / cont: %0.2f ### somave: %f ### count: %f", ve, somaVe, cont); // -0.71

		// //estadoReal[i].dAngulo = dang = (vd - ve) / DIST_ENTRE_RODAS * 180 / 3.14;***
		// estadoReal[i].dAngulo = dang = atan2((vd - ve), DIST_ENTRE_RODAS) * 180 / 3.14;
		dang = atan2((vd - ve), DIST_ENTRE_RODAS) * 180 / PI;
		printf("\ndang = atan2((vd - ve), DIST_ENTRE_RODAS): %0.2f", dang); // 0.10
		// ang = (estadoReal[i].angulo += dang); //não precisa alterar o estadoReal, pois o gazebo faz
...
void DesenhaPosicoes()
{
	...
	switch (c) {
		...
		case 't':
			...
			AlteraPosicao("home1", METADE_CAMPO_X, METADE_CAMPO_Y, 0, 0);
			AlteraPosicao("home2", METADE_CAMPO_X - 1, METADE_CAMPO_Y, 0, 0);
			AlteraPosicao("home3", METADE_CAMPO_X - 1.5, METADE_CAMPO_Y, 0, 0);
			AlteraPosicao("ball", METADE_CAMPO_X , METADE_CAMPO_Y + 1, 0, 0);
			AlteraPosicao("guest1", METADE_CAMPO_X + 1, METADE_CAMPO_Y, 0, 0);
			AlteraPosicao("guest2", METADE_CAMPO_X + 1.2, METADE_CAMPO_Y, 0, 0);
			AlteraPosicao("guest3", METADE_CAMPO_X + 1.5, METADE_CAMPO_Y, 0, 0);
			break;
...
void PoseCallback(const gazebo_msgs::ModelStates& msg)
{
	...
	string modelNames[] = {"home1", "home2", "home3", "ball", "guest1", "guest2", "guest3"}; // array para identificar os índices dos robôs e bola do model_states, portanto a ordem importa
...
int main(int argc, char** argv) {
	...
	ros::init(argc, argv, "vsss_team1_control");
	ros::NodeHandle n;

	pubAtacanteHome = n.advertise<geometry_msgs::Twist>("/cmd_vel_home3", 1);
	pubVolanteHome = n.advertise<geometry_msgs::Twist>("/cmd_vel_home2", 1);
	pubGoleiroHome = n.advertise<geometry_msgs::Twist>("/cmd_vel_home1", 1);
