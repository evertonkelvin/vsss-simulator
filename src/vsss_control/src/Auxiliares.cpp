...
void geraMatrizComb(guint8 comb[MAX_CONT_COMB][NUM_ROBOS_TIME]) {
	...
	do {
		 ...
		 * 3a combinacao		2				0				0
		 */
		for (indComb = 0; ++indsComb[indComb] == NUM_CORES_AUX && indComb < NUM_ROBOS_TIME; indComb++) {
			indsComb[indComb] = 0;
		}
...
// 	tamIdealXBlob = 4 * (limitesCampo.maxXSemGol - limitesCampo.minXSemGol) / TAM_X_CAMPO_SEM_GOL - 1; // (4cm) ideal (-2 pois os limites (contorno) nao sao vistos
// 	tamIdealYBlob = 4 * (limitesCampo.maxY - limitesCampo.minY) / TAM_Y_CAMPO - 1; // ideal;
...
// 	distIdealEtiquetas = 4 * (limitesCampo.maxXSemGol - limitesCampo.minXSemGol) / TAM_X_CAMPO_SEM_GOL;
...
// 	maxPixelsPorSegundo = (limitesCampo.maxXSemGol - limitesCampo.minXSemGol) * VELOCIDADE_MAXIMA / TAM_X_CAMPO_SEM_GOL; // 200cm/s
// }

// void FutebolCamera::converteRGB24Paleta(guint8 *pImgDigi, Cor *pImgCap, Cor *pCampoVazio) {
...
// 		int g = pImgCap->g;
// 		int b = pImgCap->b;
// 		if (abs(r - pCampoVazio->r) > nivelCampoVazio || abs(g - pCampoVazio->g) > nivelCampoVazio || abs(b - pCampoVazio->b) > nivelCampoVazio) {
...
// 	int linInicioGol = linInicio + (linFinal - linInicio) * (TAM_Y_CAMPO - TAM_Y_DO_GOL) / 2 / TAM_Y_CAMPO - 15;
// 	int linFinalGol = linInicioGol + (linFinal - linInicio) * TAM_Y_DO_GOL / TAM_Y_CAMPO + 15;
...
// 			if (aInicio[indCor] <= ax && ax <= aFinal[indCor] && bInicio[indCor] <= bx && bx <= bFinal[indCor] && lumInicio[indCor] <= lum && lum <= lumFinal[indCor]) {
// 				cor = indCor;
// 				break;
// 			}
// 		}
...
// 	limitesCampo.minXSemGol = props->getInt(strIndCam + "limitesCampo.minXSemGol", 10);
// 	limitesCampo.maxXSemGol = props->getInt(strIndCam + "limitesCampo.maxXSemGol", 620);
...
// 	try {
// 		Glib::RefPtr<Gdk::Pixbuf> imgCampVazio = Gdk::Pixbuf::create_from_file(strIndCam, 640, 480);
...
// 		inicio[i] = props->getInt(strIndCam + "InicioFaixaCanal" + canal + "Cor" + conv[i]);
// 		final[i] = props->getInt(strIndCam + "FinalFaixaCanal" + canal + "Cor" + conv[i]);
// 	}
...
// 	for (int i = 0; i < MAX_IND_CORES; i++) {
// 		props->put(strIndCam + "InicioFaixaCanal" + canal + "Cor" + conv[i], inicio[i]);
// 		props->put(strIndCam + "FinalFaixaCanal" + canal + "Cor" + conv[i], final[i]);
// 	}
// }
